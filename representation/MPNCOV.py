...
class Covpool(Function):
    """
    forward: Compute the covariance matrix of X Σ=X I_hat X^{T}
    backward: Compute dl/dX given dl/dΣ, dl/dX = (dl/dΣ+(dl/dΣ)^{T})XI_hat
    """
    @staticmethod
...
class Sqrtm(Function):
    """
    forward: Compute square root of Σ.
        pre-norm: A=1/tr(Σ) * Σ
        N-S iteration: Y_k = 0.5*Y_{k-1}(I3-Y_{k-1}Z_{k-1})
                       Z_k = 0.5*(I3-Z_{k-1}Y_{k-1})Z_{k-1}
                       s.t. Y_0=A, Z_0=I
        post_comprehension: C=sqrt(tr(Σ))*Y_{n}
    ...
    def backward(ctx, grad_output):
        """
        backward:
            post: dl|dY_{n} = sqrt(tr(Σ))*dl|dC
                  dΣ|post = 1/sqrt(tr(Σ)) * tr((dl|dC)Y_{n})I
            N-S iteration:
            pre: dl|dΣ = 1/tr(Σ)*dl|dA-1/tr(Σ)²*tr((dl|dC)Σ)I+dΣ|post
        :param ctx:
        :param grad_output:
        :return:
        """
...
class Triuvec(Function):
    ...
    def backward(ctx, grad_output):
        """
        reshape the vector to a [dim, dim] matrix
        :param ctx:
        :param grad_output:
        :return:
        """
